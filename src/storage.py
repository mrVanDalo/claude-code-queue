...
class MarkdownPromptParser:
    ...
    def parse_prompt_file(file_path: Path) -> Optional[QueuedPrompt]:
        ...
        try:
            ...
            prompt_id = file_path.stem.split('-', 1)[0] if '-' in file_path.stem else file_path.stem
...
class QueueStorage:
        ...
        if self.state_file.exists():
            try:
                ...
                if data.get('last_processed'):
                    state.last_processed = datetime.fromisoformat(data['last_processed'])
    ...
    def _load_prompts_from_files(self) -> List[QueuedPrompt]:
        ...
        for file_path in self.queue_dir.glob("*.md"):
            ...
            if (file_path.name.endswith('.executing.md') or 
                file_path.name.endswith('.rate-limited.md') or
                '#' in file_path.name):
                continue
    ...
    def _save_single_prompt(self, prompt: QueuedPrompt) -> bool:
        ...
        try:
            ...
            base_filename = f"{prompt.id}-{self._sanitize_filename(prompt.content[:50])}.md"
            ...
            if prompt.status == PromptStatus.COMPLETED:
                ...
                base_filename = f"{prompt.id}.executing.md"
            elif prompt.status == PromptStatus.RATE_LIMITED:
                target_dir = self.queue_dir
                base_filename = f"{prompt.id}.rate-limited.md"
            else:  # QUEUED
                target_dir = self.queue_dir
    ...
    def _remove_prompt_files(self, prompt_id: str, directory: Path) -> None:
        """Remove all files for a prompt ID from a directory."""
        patterns = [
            f"{prompt_id}*.md",
